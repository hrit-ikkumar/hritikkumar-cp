...
int naive_approach(int *a, int n)
{
	...
	{
		for(int end_index = 0; end_index < n; ++end_index) // ending range
		{
			int sum = 0; // each subarray's sum
			for(int range_iterator=start_index; range_iterator<=end_index; ++range_iterator) // sum of all the elements in the give range 
			{
