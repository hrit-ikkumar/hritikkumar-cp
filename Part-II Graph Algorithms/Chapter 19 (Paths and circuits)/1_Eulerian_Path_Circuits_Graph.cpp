	...
	public:
		...
		void runScript()
		{
			...
			if(checkWheaterWeCanFindEulerianPath())
			{
				...
				this->dfs(start_node); // run the dfs for eulerian path
				
				// condition for disconnected graph
				if( (signed) eulerianPath.size() == edges + 1)
				{
					cout<<"Eulerian Path: ";
					for(auto it:eulerianPath)
						cout<<it<<" ";
					cout<<endl;
				}
				else
				{
					cout<<"We didn't traverse all the edges. So, it does not contain eulerian path"<<endl;
				}
			}
			else
			{
		...
		void dfs(int root)
		{
			...
			{
				dfs(graph[root][--outDegree[root]]);
			}
			eulerianPath.push_front(root);
		}
